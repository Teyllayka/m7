                ...
                Array.prototype.forEach.call(focusableElements, function (focusableElement) {
                    focusableElement.addEventListener("focus", function () {
                        "keyboard" === handlerType &&
                            ((focusableElement.classList.contains("t-input_pvis") || focusableElement.classList.contains("t-input-phonemask")) && (focusableElement = focusableElement.parentElement),
                ...
                Array.prototype.forEach.call(fields, function (field) {
                    if ((formWrapper.classList.contains("t-form__inputsbox_inrow") || (field = field.closest(".t-input-group")), field && field.classList.contains("t-input-group_inrow-withsibling"))) {
                        ...
                        if (
                            ...
                            !(
                                (nextField.classList.contains("t-input-group_inrow") && !field.classList.contains("t-input-group_widthdef")) ||
                                (field.classList.contains("t-input-group_widthdef") && nextField.classList.contains("t-input-group") && !nextField.classList.contains("t-input-group_inrow"))
                            )
                        )
                            return field.classList.remove("t-input-group_inrow-withsibling"), void field.classList.add("t-input-group_inrow-last");
                    ...
                    nextField &&
                        ...
                        nextField.classList.contains("t-input-group_inrow-withsibling") || nextField.classList.add("t-input-group_inrow-last"),
                        nextField.classList.contains("t-input-group_inrow") && field.classList.add("t-input-group_inrow-last")),
                        formWrapper.classList.contains("t-form__inputsbox_inrow") || t_forms__calculateFieldsWidthInJS(field, formWrapperWidth);
                });
    ...
    t_forms__createArrWithAllRows(allLastFields, "t-input-group_inrow").forEach(function (row) {
        ...
        if (
            ...
            var commonWidth = row.reduce(function (sum, field) {
                var fieldClasses,
                    widthClass = field.classList.value.split(" ").filter(function (fieldClass) {
...
function t_forms__combineFieldsWithDefWidth(allLastFields, recId) {
    ...
    t_forms__createArrWithAllRows(allLastFields, "t-input-group_widthdef").forEach(function (row) {
        ...
        allRowsWithDefWidth.forEach(function (row) {
            var rowWidth,
                widthClass = "t-input-group_width" + ({ 4: "25", 3: "33", 2: "50" }[row.length] || "100");
            row.forEach(function (field) {
...
function t_forms__createArrWithAllRows(allLastFields, certainClass) {
    ...
    return (
        Array.prototype.forEach.call(allLastFields, function (field) {
            for (oneSequence.push(field); field.previousElementSibling && field.previousElementSibling.classList.contains(certainClass) && !field.previousElementSibling.classList.contains("t-input-group_inrow-last"); )
...
function t_forms__calculateFieldsWidthInJS(field, formWrapperWidth) {
    var marginLeft = 15,
        columns = { 1: "t-input-block_width100", 2: "t-input-block_width50", 3: "t-input-block_width33", 4: "t-input-block_width25" };
    ...
    if (btnSubmit) {
        ...
        if (form) {
            ...
            if ((btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus), !((btnStatus && btnStatus >= 1) || t_hasClass(btnSubmit, "t706__submit_disable")))) {
                if ((t_addClass(btnSubmit, "t-btn_sending"), (btnSubmit.tildaSendingStatus = "1"), window.tildaForm.hideErrors(form), (arrErrors = window.tildaForm.validate(form)), window.tildaForm.showErrors(form, arrErrors)))
                    return t_removeClass(btnSubmit, "t-btn_sending"), void (btnSubmit.tildaSendingStatus = "0");
                var allRecords,
                    formKey = document.getElementById("allrecords").getAttribute("data-tilda-formskey"),
                ...
                if (form.querySelector(".g-recaptcha") && grecaptcha) {
                    ...
                    if (captchaId) grecaptcha.reset(captchaId);
                        ...
                        var opts = { size: "invisible", sitekey: form.getAttribute("data-tilda-captchakey"), callback: window.tildaForm.captchaCallback };
...
function t_forms__submitEvent(event) {
    ...
    if ((event.target && (form = event.target), form)) {
        ...
        btnSubmit && btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus),
            btnStatus && "3" === btnStatus ? (btnSubmit.tildaSendingStatus = "") : (btnSubmit && !t_hasClass(btnSubmit, "t706__submit_disable") && btnSubmit.click(), event.preventDefault ? event.preventDefault() : (event.returnValue = !1));
    }
}
...
function t_parseScripts(main, className) {
    ...
    Array.prototype.forEach.call(scripts, function (oldScript) {
        ...
        if (oldScript.innerHTML.length) newScript.appendChild(document.createTextNode(oldScript.innerHTML)), oldScript.parentNode.replaceChild(newScript, oldScript);
        else {
...
function t_forms__onSuccess(form) {
    ...
    var rec = form.closest(".r"),
        ...
        successBox = form.querySelector(".t-form__successbox"),
        successBoxOffset = successBox ? successBox.getBoundingClientRect().top + window.pageYOffset : 0,
        target = 0,
...
(window.t_forms__lang = (window.navigator.userLanguage || window.navigator.language).toUpperCase().slice(0, 2)),
    ...
    (window.initForms = {}),
    (window.tildaForm = { versionLib: "02.001", endpoint: "/", isRecaptchaScriptInit: !1, currentFormProccessing: !1 }),
    t_onReady(function () {
        ...
        if (allrecords) {
            ...
            rootzoneapi && (window.tildaForm.endpoint = "/");
        }
        t_forms__initForms();
    ...
    (window.tildaForm.validate = function (form) {
        ...
        for (var inputs = form.querySelectorAll(".js-tilda-rule"), arrError = [], isEmptyValue = !0, i = 0; i < inputs.length; i++) {
            var input = inputs[i],
                ...
                quizStep = input.closest(".t-conditional"),
                isHidden;
            if (!((inputGroup && "true" === inputGroup.getAttribute("data-hidden-by-condition")) || (quizStep && "true" === quizStep.getAttribute("data-hidden-by-condition")))) {
                ...
                if ((((value.length && !isCheckBoxOrRadio) || (isCheckBoxOrRadio && isChecked)) && (isEmptyValue = !1), isReq && (isEmpty || (isCheckBoxOrRadio && !isChecked)))) objError.type.push("req");
                else {
                    switch (dataRule) {
                        case "email":
                            (regExp = /^(?!\.)(?!.*\.\.)[a-zA-Zёа-яЁА-Я0-9\u2E80-\u2FD5\u3190-\u319f\u3400-\u4DBF\u4E00-\u9FCC\uF900-\uFAAD_\.\-\+]{0,63}[a-zA-Zёа-яЁА-Я0-9\u2E80-\u2FD5\u3190-\u319f\u3400-\u4DBF\u4E00-\u9FCC\uF900-\uFAAD_\-\+]@[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,11}$/gi),
                        ...
                        case "url":
                            (regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9_\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/gi),
                                value.length &&
                                    ((strValue = (strValue = (strValue = value.split("//")) && strValue.length > 1 ? strValue[1] : strValue[0]).split("/")) && strValue.length && strValue[0]
                        ...
                        case "date":
                            t_onFuncLoad("t_datepicker__readDateFromInput", function () {
                                ...
                                valueNoSpace.length > 0 && !t_datepicker__isDateValid(date) && objError.type.push("date");
                            });
                            break;
                        case "time":
        ...
        if ("y" === dataFormCart) {
            ...
            if (isMinOrderSetted)
                if (
                    ...
                    (objError = { obj: {}, type: [] }).type.push("minorder"), arrError.push(objError);
            if (isMinQuantitySetted && window.tcart.total < window.tcart_mincntorder) (objError = { obj: {}, type: [] }).type.push("minquantity"), arrError.push(objError);
        }
                    ...
                    (strError = ""),
                        (errorItem = form.querySelector(".js-rule-error-" + error))
                            ? (errorItem.textContent && errorItem.innerText) || !localizedError || -1 !== strCommonError.indexOf(localizedError)
                                ? ((strError = errorItem.textContent || errorItem.innerText), -1 === strCommonError.indexOf(strError) && (strCommonError = strCommonError + '<p class="t-form__errorbox-item">' + strError + "</p>"))
    ...
    (window.tildaForm.showErrors = function (form, arrErrors) {
        ...
        inputBoxClassName || (inputBoxClassName = ".blockinput");
        for (var inputGroup = "", isErrorBox = !1, isShowErrors = !0, errorItem = "", errorInputs = "", strError = "", inputErrors = [], i = 0; i < arrErrors.length; i++)
            if (arrErrors[i] && arrErrors[i].obj) {
                if (0 === i && "none" === arrErrors[i].obj) {
                    ...
                    if (((strError = ""), (errorItem = form.querySelectorAll(".js-rule-error-" + error)).length))
                        for (var k = 0; k < errorItem.length; k++) {
                            ...
                            (currentErrorItem = errorItem[k]).getAttribute("data-rule-filled")
                                ...
                                : ((currentErrorItem.textContent && currentErrorItem.innerText) || !t_forms__getMsg(error)
                                      ? (strError = errorItem[0].textContent || errorItem[0].innerText)
                                      : (currentErrorItem.innerHTML = '<a href="#" class="t-form__errorbox-link">' + t_forms__getMsg(error) + "</a>"),
                                  (currentErrorItem.style.display = "block"),
                                  -1 === inputErrors.indexOf(error) && t_forms__moveToErrorInput(inputId, currentErrorItem),
                                  inputErrors.push(error));
                        }
                    else if (t_forms__getMsg(error) && (errorItem = form.querySelectorAll(".js-rule-error-all")).length)
                        for (var k = 0; k < errorItem.length; k++) {
                            var currentErrorItem;
                            ((currentErrorItem = errorItem[k]).innerHTML = '<a href="#" class="t-form__errorbox-link">' + t_forms__getMsg(error) + "</a>"),
                                (currentErrorItem.style.display = "block"),
                                -1 === inputErrors.indexOf(error) && t_forms__moveToErrorInput(inputId, currentErrorItem),
                                inputErrors.push(error);
                        }
    ...
    (window.tildaForm.addMembersInfoToForm = function (form) {
        if (form) {
            ...
            try {
                ...
                if ((mauserInfo && t_removeEl(mauserInfo), !window.mauser)) return;
                ...
                return void console.log("addMembersInfoToForm exception: ", error);
            }
        }
    }),
                            ...
                            if (data) {
                                ...
                                if ("object" == typeof objData) {
                                    ...
                                    try {
                                        ...
                                        {
                                            return (window.tildaForm.endpoint = "/"), window.tildaForm.send(form, btnSubmit, formType, formKey), !1;
                        ...
                        if (xhr.status >= 200 && xhr.status < 400) {
                            ...
                            if ((t_removeClass(btnSubmit, "t-btn_sending"), (btnSubmit.tildaSendingStatus = "0"), (data = xhr.responseText)))
                                if ("{" == data.substring(0, 1)) {
                                    var objData = JSON.parse(data);
                                    if ("object" == typeof objData)
                                        if (objData && objData.message && "OK" !== objData.message) successBox.innerHTML = objData.message;
    ...
    (function () {
        try {
            var TILDAPAGE_URL = window.location.href,
                TILDAPAGE_QUERY = "",
                TILDAPAGE_UTM = "";
            if (-1 !== TILDAPAGE_URL.toLowerCase().indexOf("utm_") && "string" == typeof (TILDAPAGE_QUERY = (TILDAPAGE_QUERY = (TILDAPAGE_URL = TILDAPAGE_URL.toLowerCase()).split("?"))[1])) {
                var arPair,
                    i,
                    arParams = TILDAPAGE_QUERY.split("&");
                for (i in arParams) "function" != typeof arParams[i] && "utm_" == (arPair = arParams[i].split("="))[0].substring(0, 4) && (TILDAPAGE_UTM = TILDAPAGE_UTM + arParams[i] + "|||");
...
function t_serializeArray(form) {
    for (var arr = [], elements = form.querySelectorAll("input, textarea, button, select"), i = 0; i < elements.length; i++) {
        ...
        if (!(!elements[i].name || elements[i].disabled || ["file", "reset", "submit", "button"].indexOf(elements[i].type) > -1 || (inputGroup && inputGroup.getAttribute("data-hidden-by-condition"))))
            if ("select-multiple" !== elements[i].type) (["checkbox", "radio"].indexOf(elements[i].type) > -1 && !elements[i].checked) || arr.push({ name: elements[i].name, value: elements[i].value });
...
function t_fadeIn(el) {
    if ("block" !== el.style.display) {
        ...
        var timer = setInterval(function () {
            (el.style.opacity = opacity), (opacity += 0.1) >= 1 && clearInterval(timer);
        }, 30);
    }
}
